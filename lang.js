/*
  Julia's Strickmuster-Editor – Sprachdatei
  Copyright (C) 2026 heino17
  https://github.com/heino17/Knitting-Pattern-Editor

  Dieses File ist bewusst als eigenständige, reine JS-Datei gehalten
  (kein separates .json/.yml + fetch), weil fetch() bei lokal per
  Doppelklick geöffneten HTML-Dateien (file://) in vielen Browsern aus
  Sicherheitsgründen (CORS) blockiert wird. <script src="lang.js"> hat
  dieses Problem nicht – bleibt also offline- und server-frei nutzbar.

  Neue Sprache hinzufügen:
  1. Neuen Eintrag nach dem Vorbild von "en" anlegen (alle Keys kopieren).
  2. Alle Werte übersetzen.
  3. In strickmuster.html im <select id="langSelect"> eine <option> ergänzen.
  Fertig – der Rest (Umschalten, Speichern der Wahl) funktioniert automatisch.
*/

// ==========================================================
// Start-Sprache
// ==========================================================
// Diese Sprache wird beim allerersten Öffnen verwendet (bzw. immer dann,
// wenn im Browser noch keine eigene Sprachwahl gespeichert ist).
// Muss einer der Codes aus I18N_AVAILABLE weiter unten sein: 'de', 'en',
// 'ru', 'es', 'fr', 'ja', 'ko', 'zh_CN'.
// Hat der Nutzer die Sprache schon einmal über das Dropdown gewechselt,
// hat diese gespeicherte Wahl weiterhin Vorrang vor dieser Einstellung.
const APPLICATION_STARTUP_LANGUAGE_CODE = "de";

const I18N = {

  de: {
    meta_title: "Julia's Strickmuster-Editor",
    app_title: "Julia's Strickmuster-Editor",
    app_title_short: "🧶 Strickmuster-Editor",
    eyebrow: "Maschenraster",
    header_hint: "Raster aufziehen, Maschen einfärben, Muster als Bild oder Datei sichern",
    header_toggle_collapse: "Kopfbereich einklappen",
    header_toggle_expand: "Kopfbereich ausklappen",

    lang_select_aria: "Sprache wählen",

    sidebar_aria: "Einstellungen",
    sidebar_toggle_collapse: "Seitenleiste einklappen",
    sidebar_toggle_expand: "Seitenleiste ausklappen",
    panel_icons_aria: "Eingeklappte Seitenleiste",

    icon_werkzeuge_title: "Werkzeuge",
    icon_werkzeuge_aria: "Werkzeuge anzeigen",
    icon_raster_title: "Raster",
    icon_raster_aria: "Raster-Einstellungen anzeigen",
    icon_farbe_title: "Farbe",
    icon_farbe_aria: "Farb-Einstellungen anzeigen",
    icon_muster_title: "Muster",
    icon_muster_aria: "Muster-Aktionen anzeigen",
    icon_ansicht_title: "Ansicht",
    icon_ansicht_aria: "Ansicht-Einstellungen anzeigen",

    group_werkzeuge_h2: "Werkzeuge",
    group_raster_h2: "Raster",
    rgb_values_label: "Farbwerte als RGB anzeigen",
    rows_label: "Reihen (Höhe)",
    cols_label: "Maschen (Breite)",
    cellsize_label: "Zellgröße",
    build_btn: "Raster erstellen",
    raster_note: "Achtung: Ändert das Raster, werden bereits gesetzte Farben zurückgesetzt.",

    group_farbe_h2: "Farbe",
    current_color_label: "Aktuelle Farbe",
    swatches_aria: "Zuletzt verwendete Farben",
    eraser_btn: "Radiergummi",

    tool_label: "Werkzeug",
    tooltoggle_aria: "Werkzeug wählen",
    tool_paint_title: "Stift (freihand malen)",
    tool_paint_label: "Stift",
    tool_area_title: "Bereich auswählen und füllen",
    tool_area_label: "Bereich",
    tool_pipette_title: "Pipette (Farbe aus dem Muster aufnehmen)",
    tool_pipette_label: "Pipette",
    werkzeug_note: "Im Bereich-Modus: Ecke anklicken, Rechteck aufziehen, loslassen zum Füllen. Mit der Pipette eine Zelle anklicken, um deren Farbe zu übernehmen.",

    cell_bg_label: "Standard Zell-Hintergrund",
    cell_bg_note: "Hintergrundfarbe für noch nicht bemalte Zellen. So bleibt bewusst „weiß“ gefärbtes Muster von leeren Zellen unterscheidbar. Wird dauerhaft gemerkt.",
    replace_label: "Farbe im Muster austauschen",
    replace_old_label: "Alte Farbe",
    replace_new_label: "Neue Farbe",
    replace_pick_title: "Aus Raster aufnehmen",
    replace_eraser_label: "Neue Farbe = Radiergummi (leer)",
    replace_btn: "Farbe austauschen",
    replace_note: "Ersetzt im gesamten Raster jede Masche mit der „Alten Farbe“ durch die „Neue Farbe“ (oder leert sie, wenn „Radiergummi“ aktiviert ist).",
    replace_none_found: "Diese Farbe kommt im aktuellen Muster nicht vor.",

    note_add_title: "Neue Notiz erstellen",
    note_drag_title: "Ziehen zum Verschieben",
    note_delete_title: "Notiz löschen",
    note_style_title: "Stil bearbeiten",
    note_resize_title: "Größe ändern",
    note_bg_label: "Hintergrund",
    note_textcolor_label: "Textfarbe",
    note_fontsize_label: "Schriftgröße",

    bgimage_btn_title: "Hintergrundbild",
    bgimage_choose: "Bild wählen …",
    bgimage_hide: "Bild ausblenden",
    bgimage_show: "Bild einblenden",
    bgimage_remove: "Bild entfernen",

    icon_farbtabelle_title: "Farbtabelle",
    icon_farbtabelle_aria: "Farbtabelle anzeigen",
    group_farbtabelle_h2: "Farbtabelle",
    palette_intro_note: "Feste, dauerhafte Farbsammlung (bis zu 49 Farben) – unabhängig von „Zuletzt verwendet“. Wird in eine eigene Datei gespeichert.",
    palette_add_color_label: "Neue Farbe",
    palette_pick_title: "Aus Raster aufnehmen",
    palette_add_btn: "Zur Farbtabelle hinzufügen",
    palette_add_grid_colors_btn: "Alle Rasterfarben übernehmen",
    palette_add_grid_colors_empty_msg: "Im Raster sind noch keine bemalten Maschen vorhanden.",
    palette_add_grid_colors_full_msg: "{added} Farbe(n) übernommen, {skipped} Farbe(n) nicht - die Farbtabelle ist mit 49 Farben voll. Bitte zuerst Platz schaffen und erneut versuchen.",
    palette_add_grid_colors_none_new_msg: "Alle Rasterfarben sind bereits in der Farbtabelle enthalten.",
    palette_add_grid_colors_success_msg: "{added} Farbe(n) in die Farbtabelle übernommen.",
    palette_count_label: "Farbtabelle",
    palette_grid_aria: "Farbtabelle",
    palette_usage_note: "Klick auf eine Farbe aktiviert sie im Stift. Klick auf das „×“ entfernt sie aus der Tabelle.",
    palette_export_btn: "Farbtabelle speichern",
    palette_import_btn: "Farbtabelle laden",
    palette_clear_btn: "Farbtabelle leeren",
    palette_dmc_names_btn: "🧵 DMC-Namensliste",
    dmc_names_modal_title: "DMC-Namensliste",
    dmc_names_modal_close_aria: "Schließen",
    dmc_names_modal_intro: "Für jede Farbe der Farbtabelle wird das jeweils ähnlichste aktive DMC-Garn angezeigt.",
    dmc_names_source_label: "Quelle",
    dmc_names_count_label: "Anzahl Farben",
    dmc_names_sort_label: "Sortieren nach",
    dmc_names_sort_original: "Ausgangsreihenfolge",
    dmc_names_sort_code: "DMC-Nr",
    dmc_names_sort_name: "Name",
    dmc_names_sort_hue: "Farbton",
    dmc_names_sort_light: "Helligkeit",
    dmc_names_copy_btn: "📋 Liste kopieren",
    dmc_names_download_btn: "💾 Als Textdatei speichern",
    dmc_names_download_html_btn: "🎨 Als HTML mit Farben speichern",
    dmc_names_no_match: "kein aktives DMC-Garn gefunden",
    dmc_names_palette_empty: "Die Farbtabelle ist noch leer.",
    dmc_names_copied_confirm: "✓ Kopiert",
    dmc_names_copy_fallback_prompt: "Kopieren fehlgeschlagen - Text hier markieren und manuell kopieren:",
    palette_full_msg: "Die Farbtabelle ist voll (maximal 49 Farben). Bitte zuerst eine Farbe entfernen.",
    palette_remove_title: "Aus Farbtabelle entfernen",
    palette_empty_hint: "Noch keine Farben in der Tabelle. Mit der Pipette oder dem Farbwähler oben Farben hinzufügen.",
    palette_confirm_clear: "Wirklich die gesamte Farbtabelle leeren?",
    page_zoom_aria: "Seiten-Zoom",
    header_saveload_aria: "Muster speichern / laden (Kopfbereich)",
    header_tools_row_aria: "Werkzeuge (Kopfbereich)",
    page_zoom_out_title: "Seite verkleinern",
    page_zoom_in_title: "Seite vergrößern",

    group_muster_h2: "Muster",
    undo_btn: "Rückgängig",
    undo_title: "Strg+Z",
    redo_btn: "Wiederherstellen",
    redo_title: "Strg+Y",
    clear_btn: "Alles leeren",
    import_image_btn: "Bild ins Raster laden",
    import_levels_label: "Farbstufen je Kanal",
    import_levels_off: "Aus",
    import_levels_note: "Reduziert die Farbtiefe auf wenige, klar unterscheidbare Farbstufen statt vieler feiner Zwischentöne - je weniger Stufen, desto gröber.",
    import_snap_palette_label: "Auf meine Farbtabelle einrasten",
    import_snap_palette_note: "Jede Masche bekommt die ähnlichste Farbe aus deiner gespeicherten Farbtabelle statt einer frei berechneten Farbe. Wird zusätzlich zu den Farbstufen angewendet.",
    import_snap_palette_empty_title: "Lege zuerst Farben in deiner Farbtabelle an, um diese Option zu nutzen.",
    import_snap_dmc_label: "Auf DMC-Farbtabelle einrasten",
    import_snap_dmc_note: "Jede Masche bekommt die ähnlichste Farbe aus der hinterlegten DMC-Garnfarbtabelle statt einer frei berechneten Farbe. Schließt sich mit \"Auf meine Farbtabelle einrasten\" gegenseitig aus.",
    edit_dmc_colors_link_title: "DMC-Farbtabelle bearbeiten (öffnet in neuem Tab)",
    import_note_before: "Das Bild wird auf das aktuelle Raster (",
    import_note_after: ", Breite × Höhe) gestreckt und in Pixelfarben umgesetzt.",
    export_json_btn: "Muster speichern",
    import_json_btn: "Muster laden",
    export_png_btn: "Als PNG exportieren",

    group_ansicht_h2: "Ansicht",
    grid10_label: "Jede 10. Linie hervorheben",
    numbers_label: "Zeilen-/Spaltenzahlen anzeigen",
    gridlines_label: "Gitterlinien anzeigen",
    ansicht_note: "Einstellungen gelten auch für den PNG-Export.",

    board_aria: "Strickraster",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · lokal im Browser, keine Serververbindung nötig",

    confirm_clear_all: "Wirklich das gesamte Muster leeren?",
    err_file_read: "Datei konnte nicht gelesen werden: ",
    err_image_load: "Bild konnte nicht geladen werden.",
    err_invalid_format: "Ungültiges Format",
  },

  en: {
    meta_title: "Julia's Knitting Pattern Editor",
    app_title: "Julia's Knitting Pattern Editor",
    app_title_short: "🧶 Knitting Pattern Editor",
    eyebrow: "Stitch grid",
    header_hint: "Set up a grid, color stitches, save your pattern as an image or file",
    header_toggle_collapse: "Collapse header",
    header_toggle_expand: "Expand header",

    lang_select_aria: "Choose language",

    sidebar_aria: "Settings",
    sidebar_toggle_collapse: "Collapse sidebar",
    sidebar_toggle_expand: "Expand sidebar",
    panel_icons_aria: "Collapsed sidebar",

    icon_werkzeuge_title: "Tools",
    icon_werkzeuge_aria: "Show tools",
    icon_raster_title: "Grid",
    icon_raster_aria: "Show grid settings",
    icon_farbe_title: "Color",
    icon_farbe_aria: "Show color settings",
    icon_muster_title: "Pattern",
    icon_muster_aria: "Show pattern actions",
    icon_ansicht_title: "View",
    icon_ansicht_aria: "Show view settings",

    group_werkzeuge_h2: "Tools",
    group_raster_h2: "Grid",
    rgb_values_label: "Show color values as RGB",
    rows_label: "Rows (height)",
    cols_label: "Stitches (width)",
    cellsize_label: "Cell size",
    build_btn: "Create grid",
    raster_note: "Warning: changing the grid resets colors that have already been set.",

    group_farbe_h2: "Color",
    current_color_label: "Current color",
    swatches_aria: "Recently used colors",
    eraser_btn: "Eraser",

    tool_label: "Tool",
    tooltoggle_aria: "Choose tool",
    tool_paint_title: "Pen (freehand painting)",
    tool_paint_label: "Pen",
    tool_area_title: "Select and fill an area",
    tool_area_label: "Area",
    tool_pipette_title: "Eyedropper (pick a color from the pattern)",
    tool_pipette_label: "Eyedropper",
    werkzeug_note: "In area mode: click a corner, drag out a rectangle, release to fill. With the eyedropper, click a stitch to pick up its color.",

    cell_bg_label: "Default cell background",
    cell_bg_note: "Background color for cells not yet painted. Keeps pattern cells deliberately colored white distinguishable from empty cells. Remembered permanently.",
    replace_label: "Replace a color in the pattern",
    replace_old_label: "Old color",
    replace_new_label: "New color",
    replace_pick_title: "Pick from grid",
    replace_eraser_label: "New color = eraser (empty)",
    replace_btn: "Replace color",
    replace_note: "Replaces every stitch with the \"old color\" throughout the whole grid with the \"new color\" (or clears it if \"eraser\" is enabled).",
    replace_none_found: "This color doesn't occur in the current pattern.",

    note_add_title: "Create new note",
    note_drag_title: "Drag to move",
    note_delete_title: "Delete note",
    note_style_title: "Edit style",
    note_resize_title: "Resize",
    note_bg_label: "Background",
    note_textcolor_label: "Text color",
    note_fontsize_label: "Font size",

    bgimage_btn_title: "Background image",
    bgimage_choose: "Choose image …",
    bgimage_hide: "Hide image",
    bgimage_show: "Show image",
    bgimage_remove: "Remove image",

    icon_farbtabelle_title: "Color palette",
    icon_farbtabelle_aria: "Show color palette",
    group_farbtabelle_h2: "Color palette",
    palette_intro_note: "A fixed, permanent color collection (up to 49 colors) – separate from \"Recently used\". Saved to its own file.",
    palette_add_color_label: "New color",
    palette_pick_title: "Pick from grid",
    palette_add_btn: "Add to color palette",
    palette_add_grid_colors_btn: "Add all grid colors",
    palette_add_grid_colors_empty_msg: "The grid doesn't contain any painted stitches yet.",
    palette_add_grid_colors_full_msg: "{added} color(s) added, {skipped} color(s) skipped - the color palette is full at 49 colors. Please make room and try again.",
    palette_add_grid_colors_none_new_msg: "All grid colors are already in the color palette.",
    palette_add_grid_colors_success_msg: "{added} color(s) added to the color palette.",
    palette_count_label: "Color palette",
    palette_grid_aria: "Color palette",
    palette_usage_note: "Click a color to activate it in the pen. Click the \"×\" to remove it from the palette.",
    palette_export_btn: "Save color palette",
    palette_import_btn: "Load color palette",
    palette_clear_btn: "Clear color palette",
    palette_dmc_names_btn: "🧵 DMC Name List",
    dmc_names_modal_title: "DMC Name List",
    dmc_names_modal_close_aria: "Close",
    dmc_names_modal_intro: "For each color in the color palette, the closest matching active DMC thread is shown.",
    dmc_names_source_label: "Source",
    dmc_names_count_label: "Number of colors",
    dmc_names_sort_label: "Sort by",
    dmc_names_sort_original: "Original order",
    dmc_names_sort_code: "DMC No.",
    dmc_names_sort_name: "Name",
    dmc_names_sort_hue: "Hue",
    dmc_names_sort_light: "Lightness",
    dmc_names_copy_btn: "📋 Copy list",
    dmc_names_download_btn: "💾 Save as text file",
    dmc_names_download_html_btn: "🎨 Save as HTML with colors",
    dmc_names_no_match: "no active DMC thread found",
    dmc_names_palette_empty: "The color palette is still empty.",
    dmc_names_copied_confirm: "✓ Copied",
    dmc_names_copy_fallback_prompt: "Copy failed - select and copy the text manually:",
    palette_full_msg: "The color palette is full (maximum 49 colors). Please remove a color first.",
    palette_remove_title: "Remove from palette",
    palette_empty_hint: "No colors in the palette yet. Add colors above using the eyedropper or the color picker.",
    palette_confirm_clear: "Really clear the entire color palette?",
    page_zoom_aria: "Page zoom",
    header_saveload_aria: "Save / load pattern (header)",
    header_tools_row_aria: "Tools (header)",
    page_zoom_out_title: "Zoom page out",
    page_zoom_in_title: "Zoom page in",

    group_muster_h2: "Pattern",
    undo_btn: "Undo",
    undo_title: "Ctrl+Z",
    redo_btn: "Redo",
    redo_title: "Ctrl+Y",
    clear_btn: "Clear all",
    import_image_btn: "Load image into grid",
    import_levels_label: "Color levels per channel",
    import_levels_off: "Off",
    import_levels_note: "Reduces the color depth to a few clearly distinct levels instead of many fine in-between tones - fewer levels give a coarser result.",
    import_snap_palette_label: "Snap to my color palette",
    import_snap_palette_note: "Each stitch gets the closest match from your saved color palette instead of a freely computed color. Applied in addition to the color levels.",
    import_snap_palette_empty_title: "Add colors to your palette first to use this option.",
    import_snap_dmc_label: "Snap to DMC color table",
    import_snap_dmc_note: "Each stitch gets the closest match from the built-in DMC thread color table instead of a freely computed color. Mutually exclusive with \"Snap to my color palette\".",
    edit_dmc_colors_link_title: "Edit DMC color table (opens in new tab)",
    import_note_before: "The image will be stretched to fit the current grid (",
    import_note_after: ", width × height) and converted to pixel colors.",
    export_json_btn: "Save pattern",
    import_json_btn: "Load pattern",
    export_png_btn: "Export as PNG",

    group_ansicht_h2: "View",
    grid10_label: "Highlight every 10th line",
    numbers_label: "Show row/column numbers",
    gridlines_label: "Show grid lines",
    ansicht_note: "Settings also apply to the PNG export.",

    board_aria: "Knitting grid",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · runs locally in the browser, no server connection needed",

    confirm_clear_all: "Really clear the entire pattern?",
    err_file_read: "Could not read file: ",
    err_image_load: "Could not load image.",
    err_invalid_format: "Invalid format",
  },

  ru: {
    meta_title: "Julia’s Редактор вязальных узоров",
    app_title: "Julia’s Редактор вязальных узоров",
    app_title_short: "🧶 Редактор вязальных узоров",
    eyebrow: "Сетка петель",
    header_hint: "Создайте сетку, закрасьте петли, сохраните узор как изображение или файл",
    header_toggle_collapse: "Свернуть заголовок",
    header_toggle_expand: "Развернуть заголовок",

    lang_select_aria: "Выбор языка",

    sidebar_aria: "Настройки",
    sidebar_toggle_collapse: "Свернуть боковую панель",
    sidebar_toggle_expand: "Развернуть боковую панель",
    panel_icons_aria: "Свёрнутая боковая панель",

    icon_werkzeuge_title: "Инструменты",
    icon_werkzeuge_aria: "Показать инструменты",
    icon_raster_title: "Сетка",
    icon_raster_aria: "Показать настройки сетки",
    icon_farbe_title: "Цвет",
    icon_farbe_aria: "Показать настройки цвета",
    icon_muster_title: "Узор",
    icon_muster_aria: "Показать действия с узором",
    icon_ansicht_title: "Вид",
    icon_ansicht_aria: "Показать настройки вида",

    group_werkzeuge_h2: "Инструменты",
    group_raster_h2: "Сетка",
    rgb_values_label: "Показывать цвета в формате RGB",
    rows_label: "Ряды (высота)",
    cols_label: "Петли (ширина)",
    cellsize_label: "Размер клетки",
    build_btn: "Создать сетку",
    raster_note: "Внимание: изменение сетки сбрасывает уже заданные цвета.",

    group_farbe_h2: "Цвет",
    current_color_label: "Текущий цвет",
    swatches_aria: "Последние использованные цвета",
    eraser_btn: "Ластик",

    tool_label: "Инструмент",
    tooltoggle_aria: "Выбор инструмента",
    tool_paint_title: "Карандаш (рисование от руки)",
    tool_paint_label: "Карандаш",
    tool_area_title: "Выбрать и заполнить область",
    tool_area_label: "Область",
    tool_pipette_title: "Пипетка (взять цвет из узора)",
    tool_pipette_label: "Пипетка",
    werkzeug_note: "В режиме области: щёлкните по углу, растяните прямоугольник, отпустите для заполнения. Пипеткой щёлкните по клетке, чтобы взять её цвет.",

    cell_bg_label: "Фон пустой ячейки по умолчанию",
    cell_bg_note: "Цвет фона ещё не закрашенных ячеек. Так специально выбеленные ячейки узора остаются отличимыми от пустых. Сохраняется постоянно.",
    replace_label: "Заменить цвет в узоре",
    replace_old_label: "Старый цвет",
    replace_new_label: "Новый цвет",
    replace_pick_title: "Взять из сетки",
    replace_eraser_label: "Новый цвет = ластик (пусто)",
    replace_btn: "Заменить цвет",
    replace_note: "Заменяет во всей сетке каждую петлю со «старым цветом» на «новый цвет» (или очищает её, если включён «ластик»).",
    replace_none_found: "Этот цвет не встречается в текущем узоре.",

    note_add_title: "Создать новую заметку",
    note_drag_title: "Перетащите, чтобы переместить",
    note_delete_title: "Удалить заметку",
    note_style_title: "Изменить стиль",
    note_resize_title: "Изменить размер",
    note_bg_label: "Фон",
    note_textcolor_label: "Цвет текста",
    note_fontsize_label: "Размер шрифта",

    bgimage_btn_title: "Фоновое изображение",
    bgimage_choose: "Выбрать изображение …",
    bgimage_hide: "Скрыть изображение",
    bgimage_show: "Показать изображение",
    bgimage_remove: "Удалить изображение",

    icon_farbtabelle_title: "Палитра цветов",
    icon_farbtabelle_aria: "Показать палитру цветов",
    group_farbtabelle_h2: "Палитра цветов",
    palette_intro_note: "Постоянная коллекция цветов (до 49 цветов) – отдельно от «Недавно использованных». Сохраняется в отдельный файл.",
    palette_add_color_label: "Новый цвет",
    palette_pick_title: "Взять из сетки",
    palette_add_btn: "Добавить в палитру",
    palette_add_grid_colors_btn: "Добавить все цвета сетки",
    palette_add_grid_colors_empty_msg: "В сетке ещё нет закрашенных клеток.",
    palette_add_grid_colors_full_msg: "Добавлено {added} цвет(ов), пропущено {skipped} - палитра заполнена (49 цветов). Освободите место и повторите попытку.",
    palette_add_grid_colors_none_new_msg: "Все цвета сетки уже есть в палитре.",
    palette_add_grid_colors_success_msg: "В палитру добавлено {added} цвет(ов).",
    palette_count_label: "Палитра цветов",
    palette_grid_aria: "Палитра цветов",
    palette_usage_note: "Нажмите на цвет, чтобы активировать его в инструменте «Карандаш». Нажмите «×», чтобы удалить его из палитры.",
    palette_export_btn: "Сохранить палитру",
    palette_import_btn: "Загрузить палитру",
    palette_clear_btn: "Очистить палитру",
    palette_dmc_names_btn: "🧵 Список названий DMC",
    dmc_names_modal_title: "Список названий DMC",
    dmc_names_modal_close_aria: "Закрыть",
    dmc_names_modal_intro: "Для каждого цвета палитры показана ближайшая активная нить DMC.",
    dmc_names_source_label: "Источник",
    dmc_names_count_label: "Количество цветов",
    dmc_names_sort_label: "Сортировать по",
    dmc_names_sort_original: "Исходный порядок",
    dmc_names_sort_code: "№ DMC",
    dmc_names_sort_name: "Названию",
    dmc_names_sort_hue: "Оттенку",
    dmc_names_sort_light: "Светлоте",
    dmc_names_copy_btn: "📋 Скопировать список",
    dmc_names_download_btn: "💾 Сохранить как текстовый файл",
    dmc_names_download_html_btn: "🎨 Сохранить как HTML с цветами",
    dmc_names_no_match: "активная нить DMC не найдена",
    dmc_names_palette_empty: "Палитра цветов пока пуста.",
    dmc_names_copied_confirm: "✓ Скопировано",
    dmc_names_copy_fallback_prompt: "Не удалось скопировать - выделите и скопируйте текст вручную:",
    palette_full_msg: "Палитра заполнена (максимум 49 цветов). Сначала удалите какой-нибудь цвет.",
    palette_remove_title: "Удалить из палитры",
    palette_empty_hint: "В палитре пока нет цветов. Добавьте цвета выше с помощью пипетки или палитры цветов.",
    palette_confirm_clear: "Действительно очистить всю палитру цветов?",
    page_zoom_aria: "Масштаб страницы",
    header_saveload_aria: "Сохранить / загрузить узор (шапка)",
    header_tools_row_aria: "Инструменты (шапка)",
    page_zoom_out_title: "Уменьшить масштаб страницы",
    page_zoom_in_title: "Увеличить масштаб страницы",

    group_muster_h2: "Узор",
    undo_btn: "Отменить",
    undo_title: "Ctrl+Z",
    redo_btn: "Повторить",
    redo_title: "Ctrl+Y",
    clear_btn: "Очистить всё",
    import_image_btn: "Загрузить изображение в сетку",
    import_levels_label: "Уровни цвета на канал",
    import_levels_off: "Выкл",
    import_levels_note: "Уменьшает глубину цвета до нескольких четко различимых уровней вместо множества тонких промежуточных оттенков - чем меньше уровней, тем грубее результат.",
    import_snap_palette_label: "Привязать к моей цветовой палитре",
    import_snap_palette_note: "Каждая клетка получает ближайший цвет из вашей сохранённой палитры вместо свободно вычисленного цвета. Применяется в дополнение к уровням цвета.",
    import_snap_palette_empty_title: "Сначала добавьте цвета в свою палитру, чтобы использовать эту функцию.",
    import_snap_dmc_label: "Привязать к таблице цветов DMC",
    import_snap_dmc_note: "Каждая клетка получает ближайший цвет из встроенной таблицы цветов ниток DMC вместо свободно вычисленного цвета. Взаимоисключающе с «Привязать к моей цветовой палитре».",
    edit_dmc_colors_link_title: "Редактировать таблицу цветов DMC (откроется в новой вкладке)",
    import_note_before: "Изображение будет растянуто на текущую сетку (",
    import_note_after: ", ширина × высота) и преобразовано в цвета пикселей.",
    export_json_btn: "Сохранить узор",
    import_json_btn: "Загрузить узор",
    export_png_btn: "Экспортировать как PNG",

    group_ansicht_h2: "Вид",
    grid10_label: "Выделять каждую 10-ю линию",
    numbers_label: "Показывать номера рядов/столбцов",
    gridlines_label: "Показывать линии сетки",
    ansicht_note: "Настройки также применяются при экспорте в PNG.",

    board_aria: "Сетка для вязания",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · работает локально в браузере, подключение к серверу не требуется",

    confirm_clear_all: "Действительно очистить весь узор?",
    err_file_read: "Не удалось прочитать файл: ",
    err_image_load: "Не удалось загрузить изображение.",
    err_invalid_format: "Неверный формат",
  },

  es: {
    meta_title: "Julia’s Editor de Patrones de Tejido",
    app_title: "Julia’s Editor de Patrones de Tejido",
    app_title_short: "🧶 Editor de Patrones de Tejido",
    eyebrow: "Cuadrícula de puntos",
    header_hint: "Crea una cuadrícula, colorea los puntos, guarda tu patrón como imagen o archivo",
    header_toggle_collapse: "Contraer encabezado",
    header_toggle_expand: "Expandir encabezado",

    lang_select_aria: "Elegir idioma",

    sidebar_aria: "Ajustes",
    sidebar_toggle_collapse: "Contraer barra lateral",
    sidebar_toggle_expand: "Expandir barra lateral",
    panel_icons_aria: "Barra lateral contraída",

    icon_werkzeuge_title: "Herramientas",
    icon_werkzeuge_aria: "Mostrar herramientas",
    icon_raster_title: "Cuadrícula",
    icon_raster_aria: "Mostrar ajustes de cuadrícula",
    icon_farbe_title: "Color",
    icon_farbe_aria: "Mostrar ajustes de color",
    icon_muster_title: "Patrón",
    icon_muster_aria: "Mostrar acciones del patrón",
    icon_ansicht_title: "Vista",
    icon_ansicht_aria: "Mostrar ajustes de vista",

    group_werkzeuge_h2: "Herramientas",
    group_raster_h2: "Cuadrícula",
    rgb_values_label: "Mostrar valores de color como RGB",
    rows_label: "Filas (alto)",
    cols_label: "Puntos (ancho)",
    cellsize_label: "Tamaño de celda",
    build_btn: "Crear cuadrícula",
    raster_note: "Atención: cambiar la cuadrícula reinicia los colores ya establecidos.",

    group_farbe_h2: "Color",
    current_color_label: "Color actual",
    swatches_aria: "Colores usados recientemente",
    eraser_btn: "Borrador",

    tool_label: "Herramienta",
    tooltoggle_aria: "Elegir herramienta",
    tool_paint_title: "Lápiz (pintura a mano libre)",
    tool_paint_label: "Lápiz",
    tool_area_title: "Seleccionar y rellenar un área",
    tool_area_label: "Área",
    tool_pipette_title: "Cuentagotas (tomar un color del patrón)",
    tool_pipette_label: "Cuentagotas",
    werkzeug_note: "En modo área: haz clic en una esquina, arrastra un rectángulo y suelta para rellenar. Con el cuentagotas, haz clic en una celda para tomar su color.",

    cell_bg_label: "Fondo predeterminado de celda",
    cell_bg_note: "Color de fondo de las celdas aún sin pintar. Así las celdas pintadas deliberadamente de blanco siguen siendo distinguibles de las vacías. Se recuerda de forma permanente.",
    replace_label: "Reemplazar un color del patrón",
    replace_old_label: "Color antiguo",
    replace_new_label: "Color nuevo",
    replace_pick_title: "Tomar de la cuadrícula",
    replace_eraser_label: "Color nuevo = borrador (vacío)",
    replace_btn: "Reemplazar color",
    replace_note: "Reemplaza en toda la cuadrícula cada punto con el «color antiguo» por el «color nuevo» (o lo vacía si el «borrador» está activado).",
    replace_none_found: "Este color no aparece en el patrón actual.",

    note_add_title: "Crear nueva nota",
    note_drag_title: "Arrastrar para mover",
    note_delete_title: "Eliminar nota",
    note_style_title: "Editar estilo",
    note_resize_title: "Cambiar tamaño",
    note_bg_label: "Fondo",
    note_textcolor_label: "Color de texto",
    note_fontsize_label: "Tamaño de fuente",

    bgimage_btn_title: "Imagen de fondo",
    bgimage_choose: "Elegir imagen …",
    bgimage_hide: "Ocultar imagen",
    bgimage_show: "Mostrar imagen",
    bgimage_remove: "Quitar imagen",

    icon_farbtabelle_title: "Tabla de colores",
    icon_farbtabelle_aria: "Mostrar tabla de colores",
    group_farbtabelle_h2: "Tabla de colores",
    palette_intro_note: "Una colección de colores fija y permanente (hasta 49 colores), independiente de \"Usados recientemente\". Se guarda en un archivo propio.",
    palette_add_color_label: "Nuevo color",
    palette_pick_title: "Tomar de la cuadrícula",
    palette_add_btn: "Añadir a la tabla de colores",
    palette_add_grid_colors_btn: "Añadir todos los colores de la cuadrícula",
    palette_add_grid_colors_empty_msg: "La cuadrícula aún no tiene casillas pintadas.",
    palette_add_grid_colors_full_msg: "Se añadieron {added} color(es), se omitieron {skipped} - la tabla de colores está llena (49 colores). Elimina alguno e inténtalo de nuevo.",
    palette_add_grid_colors_none_new_msg: "Todos los colores de la cuadrícula ya están en la tabla de colores.",
    palette_add_grid_colors_success_msg: "Se añadieron {added} color(es) a la tabla de colores.",
    palette_count_label: "Tabla de colores",
    palette_grid_aria: "Tabla de colores",
    palette_usage_note: "Haz clic en un color para activarlo en el lápiz. Haz clic en la «×» para eliminarlo de la tabla.",
    palette_export_btn: "Guardar tabla de colores",
    palette_import_btn: "Cargar tabla de colores",
    palette_clear_btn: "Vaciar tabla de colores",
    palette_dmc_names_btn: "🧵 Lista de nombres DMC",
    dmc_names_modal_title: "Lista de nombres DMC",
    dmc_names_modal_close_aria: "Cerrar",
    dmc_names_modal_intro: "Para cada color de la tabla de colores se muestra el hilo DMC activo más parecido.",
    dmc_names_source_label: "Origen",
    dmc_names_count_label: "Número de colores",
    dmc_names_sort_label: "Ordenar por",
    dmc_names_sort_original: "Orden original",
    dmc_names_sort_code: "N.º DMC",
    dmc_names_sort_name: "Nombre",
    dmc_names_sort_hue: "Tono",
    dmc_names_sort_light: "Luminosidad",
    dmc_names_copy_btn: "📋 Copiar lista",
    dmc_names_download_btn: "💾 Guardar como archivo de texto",
    dmc_names_download_html_btn: "🎨 Guardar como HTML con colores",
    dmc_names_no_match: "no se encontró ningún hilo DMC activo",
    dmc_names_palette_empty: "La tabla de colores todavía está vacía.",
    dmc_names_copied_confirm: "✓ Copiado",
    dmc_names_copy_fallback_prompt: "No se pudo copiar - selecciona y copia el texto manualmente:",
    palette_full_msg: "La tabla de colores está llena (máximo 49 colores). Elimina un color primero.",
    palette_remove_title: "Eliminar de la tabla",
    palette_empty_hint: "Aún no hay colores en la tabla. Añade colores arriba con el cuentagotas o el selector de color.",
    palette_confirm_clear: "¿Seguro que quieres vaciar toda la tabla de colores?",
    page_zoom_aria: "Zoom de página",
    header_saveload_aria: "Guardar / cargar patrón (encabezado)",
    header_tools_row_aria: "Herramientas (encabezado)",
    page_zoom_out_title: "Reducir zoom de página",
    page_zoom_in_title: "Aumentar zoom de página",

    group_muster_h2: "Patrón",
    undo_btn: "Deshacer",
    undo_title: "Ctrl+Z",
    redo_btn: "Rehacer",
    redo_title: "Ctrl+Y",
    clear_btn: "Borrar todo",
    import_image_btn: "Cargar imagen en la cuadrícula",
    import_levels_label: "Niveles de color por canal",
    import_levels_off: "Desactivado",
    import_levels_note: "Reduce la profundidad de color a unos pocos niveles claramente distintos en lugar de muchos tonos intermedios finos - menos niveles dan un resultado más tosco.",
    import_snap_palette_label: "Ajustar a mi paleta de colores",
    import_snap_palette_note: "Cada casilla recibe el color más parecido de tu paleta guardada en lugar de un color calculado libremente. Se aplica además de los niveles de color.",
    import_snap_palette_empty_title: "Añade colores a tu paleta primero para usar esta opción.",
    import_snap_dmc_label: "Ajustar a la tabla de colores DMC",
    import_snap_dmc_note: "Cada casilla recibe el color más parecido de la tabla de colores de hilo DMC incorporada, en lugar de un color calculado libremente. Se excluye mutuamente con «Ajustar a mi paleta de colores».",
    edit_dmc_colors_link_title: "Editar tabla de colores DMC (se abre en una pestaña nueva)",
    import_note_before: "La imagen se ajustará a la cuadrícula actual (",
    import_note_after: ", ancho × alto) y se convertirá en colores de píxeles.",
    export_json_btn: "Guardar patrón",
    import_json_btn: "Cargar patrón",
    export_png_btn: "Exportar como PNG",

    group_ansicht_h2: "Vista",
    grid10_label: "Resaltar cada 10.ª línea",
    numbers_label: "Mostrar números de fila/columna",
    gridlines_label: "Mostrar líneas de la cuadrícula",
    ansicht_note: "Los ajustes también se aplican a la exportación PNG.",

    board_aria: "Cuadrícula de punto",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · funciona localmente en el navegador, no se necesita conexión a un servidor",

    confirm_clear_all: "¿Seguro que quieres borrar todo el patrón?",
    err_file_read: "No se pudo leer el archivo: ",
    err_image_load: "No se pudo cargar la imagen.",
    err_invalid_format: "Formato no válido",
  },

  fr: {
    meta_title: "Julia’s Éditeur de Modèles de Tricot",
    app_title: "Julia’s Éditeur de Modèles de Tricot",
    app_title_short: "🧶 Éditeur de Modèles de Tricot",
    eyebrow: "Grille de mailles",
    header_hint: "Créez une grille, coloriez les mailles, enregistrez votre motif en image ou en fichier",
    header_toggle_collapse: "Réduire l'en-tête",
    header_toggle_expand: "Développer l'en-tête",

    lang_select_aria: "Choisir la langue",

    sidebar_aria: "Paramètres",
    sidebar_toggle_collapse: "Réduire la barre latérale",
    sidebar_toggle_expand: "Développer la barre latérale",
    panel_icons_aria: "Barre latérale réduite",

    icon_werkzeuge_title: "Outils",
    icon_werkzeuge_aria: "Afficher les outils",
    icon_raster_title: "Grille",
    icon_raster_aria: "Afficher les paramètres de la grille",
    icon_farbe_title: "Couleur",
    icon_farbe_aria: "Afficher les paramètres de couleur",
    icon_muster_title: "Motif",
    icon_muster_aria: "Afficher les actions du motif",
    icon_ansicht_title: "Affichage",
    icon_ansicht_aria: "Afficher les paramètres d'affichage",

    group_werkzeuge_h2: "Outils",
    group_raster_h2: "Grille",
    rgb_values_label: "Afficher les couleurs en RGB",
    rows_label: "Rangs (hauteur)",
    cols_label: "Mailles (largeur)",
    cellsize_label: "Taille des cases",
    build_btn: "Créer la grille",
    raster_note: "Attention : modifier la grille réinitialise les couleurs déjà définies.",

    group_farbe_h2: "Couleur",
    current_color_label: "Couleur actuelle",
    swatches_aria: "Couleurs récemment utilisées",
    eraser_btn: "Gomme",

    tool_label: "Outil",
    tooltoggle_aria: "Choisir l'outil",
    tool_paint_title: "Crayon (dessin à main levée)",
    tool_paint_label: "Crayon",
    tool_area_title: "Sélectionner et remplir une zone",
    tool_area_label: "Zone",
    tool_pipette_title: "Pipette (prélever une couleur du motif)",
    tool_pipette_label: "Pipette",
    werkzeug_note: "En mode zone : cliquez sur un coin, tracez un rectangle, relâchez pour remplir. Avec la pipette, cliquez sur une case pour en récupérer la couleur.",

    cell_bg_label: "Fond de cellule par défaut",
    cell_bg_note: "Couleur de fond des cellules pas encore peintes. Permet de distinguer les cellules volontairement peintes en blanc des cellules vides. Mémorisé de façon permanente.",
    replace_label: "Remplacer une couleur du motif",
    replace_old_label: "Ancienne couleur",
    replace_new_label: "Nouvelle couleur",
    replace_pick_title: "Prélever depuis la grille",
    replace_eraser_label: "Nouvelle couleur = gomme (vide)",
    replace_btn: "Remplacer la couleur",
    replace_note: "Remplace dans toute la grille chaque maille avec l'« ancienne couleur » par la « nouvelle couleur » (ou la vide si la « gomme » est activée).",
    replace_none_found: "Cette couleur n'apparaît pas dans le motif actuel.",

    note_add_title: "Créer une nouvelle note",
    note_drag_title: "Glisser pour déplacer",
    note_delete_title: "Supprimer la note",
    note_style_title: "Modifier le style",
    note_resize_title: "Redimensionner",
    note_bg_label: "Arrière-plan",
    note_textcolor_label: "Couleur du texte",
    note_fontsize_label: "Taille de police",

    bgimage_btn_title: "Image de fond",
    bgimage_choose: "Choisir une image …",
    bgimage_hide: "Masquer l'image",
    bgimage_show: "Afficher l'image",
    bgimage_remove: "Supprimer l'image",

    icon_farbtabelle_title: "Palette de couleurs",
    icon_farbtabelle_aria: "Afficher la palette de couleurs",
    group_farbtabelle_h2: "Palette de couleurs",
    palette_intro_note: "Une collection de couleurs fixe et permanente (jusqu'à 49 couleurs), distincte des « Couleurs récentes ». Enregistrée dans un fichier séparé.",
    palette_add_color_label: "Nouvelle couleur",
    palette_pick_title: "Prélever sur la grille",
    palette_add_btn: "Ajouter à la palette",
    palette_add_grid_colors_btn: "Ajouter toutes les couleurs de la grille",
    palette_add_grid_colors_empty_msg: "La grille ne contient encore aucune case peinte.",
    palette_add_grid_colors_full_msg: "{added} couleur(s) ajoutée(s), {skipped} ignorée(s) - la palette est pleine (49 couleurs). Libérez de la place puis réessayez.",
    palette_add_grid_colors_none_new_msg: "Toutes les couleurs de la grille sont déjà dans la palette.",
    palette_add_grid_colors_success_msg: "{added} couleur(s) ajoutée(s) à la palette.",
    palette_count_label: "Palette de couleurs",
    palette_grid_aria: "Palette de couleurs",
    palette_usage_note: "Cliquez sur une couleur pour l'activer dans le crayon. Cliquez sur « × » pour la retirer de la palette.",
    palette_export_btn: "Enregistrer la palette",
    palette_import_btn: "Charger la palette",
    palette_clear_btn: "Vider la palette",
    palette_dmc_names_btn: "🧵 Liste des noms DMC",
    dmc_names_modal_title: "Liste des noms DMC",
    dmc_names_modal_close_aria: "Fermer",
    dmc_names_modal_intro: "Pour chaque couleur de la palette, le fil DMC actif le plus proche est indiqué.",
    dmc_names_source_label: "Source",
    dmc_names_count_label: "Nombre de couleurs",
    dmc_names_sort_label: "Trier par",
    dmc_names_sort_original: "Ordre d'origine",
    dmc_names_sort_code: "N° DMC",
    dmc_names_sort_name: "Nom",
    dmc_names_sort_hue: "Teinte",
    dmc_names_sort_light: "Luminosité",
    dmc_names_copy_btn: "📋 Copier la liste",
    dmc_names_download_btn: "💾 Enregistrer en fichier texte",
    dmc_names_download_html_btn: "🎨 Enregistrer en HTML avec les couleurs",
    dmc_names_no_match: "aucun fil DMC actif trouvé",
    dmc_names_palette_empty: "La palette de couleurs est encore vide.",
    dmc_names_copied_confirm: "✓ Copié",
    dmc_names_copy_fallback_prompt: "Échec de la copie - sélectionnez et copiez le texte manuellement :",
    palette_full_msg: "La palette est pleine (49 couleurs maximum). Veuillez d'abord retirer une couleur.",
    palette_remove_title: "Retirer de la palette",
    palette_empty_hint: "Aucune couleur dans la palette pour l'instant. Ajoutez des couleurs ci-dessus avec la pipette ou le sélecteur.",
    palette_confirm_clear: "Vraiment vider toute la palette de couleurs ?",
    page_zoom_aria: "Zoom de la page",
    header_saveload_aria: "Enregistrer / charger le motif (en-tête)",
    header_tools_row_aria: "Outils (en-tête)",
    page_zoom_out_title: "Réduire le zoom de la page",
    page_zoom_in_title: "Augmenter le zoom de la page",

    group_muster_h2: "Motif",
    undo_btn: "Annuler",
    undo_title: "Ctrl+Z",
    redo_btn: "Rétablir",
    redo_title: "Ctrl+Y",
    clear_btn: "Tout effacer",
    import_image_btn: "Charger une image dans la grille",
    import_levels_label: "Niveaux de couleur par canal",
    import_levels_off: "Désactivé",
    import_levels_note: "Réduit la profondeur de couleur à quelques niveaux clairement distincts au lieu de nombreuses teintes intermédiaires fines - moins de niveaux donnent un résultat plus grossier.",
    import_snap_palette_label: "Aligner sur ma palette de couleurs",
    import_snap_palette_note: "Chaque case reçoit la couleur la plus proche de votre palette enregistrée au lieu d'une couleur calculée librement. S'applique en plus des niveaux de couleur.",
    import_snap_palette_empty_title: "Ajoutez d'abord des couleurs à votre palette pour utiliser cette option.",
    import_snap_dmc_label: "Aligner sur la table de couleurs DMC",
    import_snap_dmc_note: "Chaque case reçoit la couleur la plus proche de la table de couleurs de fils DMC intégrée, au lieu d'une couleur calculée librement. S'exclut mutuellement avec « Aligner sur ma palette de couleurs ».",
    edit_dmc_colors_link_title: "Modifier la table de couleurs DMC (s'ouvre dans un nouvel onglet)",
    import_note_before: "L'image sera adaptée à la grille actuelle (",
    import_note_after: ", largeur × hauteur) et convertie en couleurs de pixels.",
    export_json_btn: "Enregistrer le motif",
    import_json_btn: "Charger le motif",
    export_png_btn: "Exporter en PNG",

    group_ansicht_h2: "Affichage",
    grid10_label: "Mettre en évidence chaque 10e ligne",
    numbers_label: "Afficher les numéros de rangs/mailles",
    gridlines_label: "Afficher les lignes de la grille",
    ansicht_note: "Les paramètres s'appliquent aussi à l'export PNG.",

    board_aria: "Grille de tricot",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · fonctionne localement dans le navigateur, aucune connexion serveur nécessaire",

    confirm_clear_all: "Vraiment effacer tout le motif ?",
    err_file_read: "Impossible de lire le fichier : ",
    err_image_load: "Impossible de charger l'image.",
    err_invalid_format: "Format invalide",
  },

  ja: {
    meta_title: "Julia’s 編み模様エディター",
    app_title: "Julia’s 編み模様エディター",
    app_title_short: "🧶 編み模様エディター",
    eyebrow: "編み目グリッド",
    header_hint: "グリッドを作成し、編み目に色を付けて、画像やファイルとして保存しましょう",
    header_toggle_collapse: "ヘッダーを折りたたむ",
    header_toggle_expand: "ヘッダーを展開する",

    lang_select_aria: "言語を選択",

    sidebar_aria: "設定",
    sidebar_toggle_collapse: "サイドバーを折りたたむ",
    sidebar_toggle_expand: "サイドバーを展開する",
    panel_icons_aria: "折りたたまれたサイドバー",

    icon_werkzeuge_title: "ツール",
    icon_werkzeuge_aria: "ツールを表示",
    icon_raster_title: "グリッド",
    icon_raster_aria: "グリッド設定を表示",
    icon_farbe_title: "色",
    icon_farbe_aria: "色の設定を表示",
    icon_muster_title: "パターン",
    icon_muster_aria: "パターン操作を表示",
    icon_ansicht_title: "表示",
    icon_ansicht_aria: "表示設定を表示",

    group_werkzeuge_h2: "ツール",
    group_raster_h2: "グリッド",
    rgb_values_label: "色の値をRGBで表示",
    rows_label: "段数(高さ)",
    cols_label: "目数(幅)",
    cellsize_label: "セルサイズ",
    build_btn: "グリッドを作成",
    raster_note: "注意: グリッドを変更すると、設定済みの色はリセットされます。",

    group_farbe_h2: "色",
    current_color_label: "現在の色",
    swatches_aria: "最近使った色",
    eraser_btn: "消しゴム",

    tool_label: "ツール",
    tooltoggle_aria: "ツールを選択",
    tool_paint_title: "ペン(フリーハンドで塗る)",
    tool_paint_label: "ペン",
    tool_area_title: "範囲を選択して塗りつぶす",
    tool_area_label: "範囲",
    tool_pipette_title: "スポイト(パターンから色を取得)",
    tool_pipette_label: "スポイト",
    werkzeug_note: "範囲モード:角をクリックして矩形をドラッグし、離すと塗りつぶされます。スポイトでは、セルをクリックしてその色を取得します。",

    cell_bg_label: "セルの既定背景色",
    cell_bg_note: "まだ塗られていないセルの背景色です。意図的に白く塗ったセルと空のセルを区別できます。設定は永続的に保存されます。",
    replace_label: "パターン内の色を置き換え",
    replace_old_label: "元の色",
    replace_new_label: "新しい色",
    replace_pick_title: "グリッドから取得",
    replace_eraser_label: "新しい色 = 消しゴム(空)",
    replace_btn: "色を置き換える",
    replace_note: "グリッド全体で「元の色」のすべての編み目を「新しい色」に置き換えます(「消しゴム」が有効な場合は空にします)。",
    replace_none_found: "この色は現在のパターンには存在しません。",

    note_add_title: "新しいメモを作成",
    note_drag_title: "ドラッグして移動",
    note_delete_title: "メモを削除",
    note_style_title: "スタイルを編集",
    note_resize_title: "サイズを変更",
    note_bg_label: "背景色",
    note_textcolor_label: "文字色",
    note_fontsize_label: "文字サイズ",

    bgimage_btn_title: "背景画像",
    bgimage_choose: "画像を選択 …",
    bgimage_hide: "画像を非表示",
    bgimage_show: "画像を表示",
    bgimage_remove: "画像を削除",

    icon_farbtabelle_title: "カラーパレット",
    icon_farbtabelle_aria: "カラーパレットを表示",
    group_farbtabelle_h2: "カラーパレット",
    palette_intro_note: "「最近使用した色」とは別の、固定の永続的なカラーコレクション（最大49色）です。専用のファイルに保存されます。",
    palette_add_color_label: "新しい色",
    palette_pick_title: "グリッドから採取",
    palette_add_btn: "カラーパレットに追加",
    palette_add_grid_colors_btn: "グリッドの色をすべて追加",
    palette_add_grid_colors_empty_msg: "グリッドにはまだ色を塗られたマスがありません。",
    palette_add_grid_colors_full_msg: "{added}色を追加、{skipped}色はスキップしました - カラーパレットが満杯です（49色）。空きを作ってから再度お試しください。",
    palette_add_grid_colors_none_new_msg: "グリッドの色はすべて既にカラーパレットに含まれています。",
    palette_add_grid_colors_success_msg: "{added}色をカラーパレットに追加しました。",
    palette_count_label: "カラーパレット",
    palette_grid_aria: "カラーパレット",
    palette_usage_note: "色をクリックするとペンに適用されます。「×」をクリックするとパレットから削除されます。",
    palette_export_btn: "カラーパレットを保存",
    palette_import_btn: "カラーパレットを読み込む",
    palette_clear_btn: "カラーパレットを空にする",
    palette_dmc_names_btn: "🧵 DMCネームリスト",
    dmc_names_modal_title: "DMCネームリスト",
    dmc_names_modal_close_aria: "閉じる",
    dmc_names_modal_intro: "カラーパレットの各色について、最も近い有効なDMC刺繍糸を表示します。",
    dmc_names_source_label: "元ファイル",
    dmc_names_count_label: "色の数",
    dmc_names_sort_label: "並び替え",
    dmc_names_sort_original: "元の順序",
    dmc_names_sort_code: "DMC番号",
    dmc_names_sort_name: "名前",
    dmc_names_sort_hue: "色相",
    dmc_names_sort_light: "明るさ",
    dmc_names_copy_btn: "📋 リストをコピー",
    dmc_names_download_btn: "💾 テキストファイルとして保存",
    dmc_names_download_html_btn: "🎨 色付きHTMLとして保存",
    dmc_names_no_match: "有効なDMC糸が見つかりません",
    dmc_names_palette_empty: "カラーパレットはまだ空です。",
    dmc_names_copied_confirm: "✓ コピーしました",
    dmc_names_copy_fallback_prompt: "コピーに失敗しました - 以下のテキストを選択して手動でコピーしてください：",
    palette_full_msg: "カラーパレットが満杯です（最大49色）。先に色を削除してください。",
    palette_remove_title: "パレットから削除",
    palette_empty_hint: "まだパレットに色がありません。上のスポイトまたはカラーピッカーで色を追加してください。",
    palette_confirm_clear: "本当にカラーパレット全体を空にしますか?",
    page_zoom_aria: "ページの拡大縮小",
    header_saveload_aria: "パターンの保存・読み込み（ヘッダー）",
    header_tools_row_aria: "ツール（ヘッダー）",
    page_zoom_out_title: "ページを縮小",
    page_zoom_in_title: "ページを拡大",

    group_muster_h2: "パターン",
    undo_btn: "元に戻す",
    undo_title: "Ctrl+Z",
    redo_btn: "やり直す",
    redo_title: "Ctrl+Y",
    clear_btn: "すべて消去",
    import_image_btn: "画像をグリッドに読み込む",
    import_levels_label: "チャンネルごとの色数",
    import_levels_off: "オフ",
    import_levels_note: "多くの微妙な中間色の代わりに、はっきり区別できる少数の色数に減らします。数が少ないほど、より粗い仕上がりになります。",
    import_snap_palette_label: "自分のカラーパレットに合わせる",
    import_snap_palette_note: "各マスに、自由に計算された色の代わりに、保存したカラーパレットの中で最も近い色が割り当てられます。色数の設定に加えて適用されます。",
    import_snap_palette_empty_title: "このオプションを使うには、まずカラーパレットに色を追加してください。",
    import_snap_dmc_label: "DMCカラーテーブルに合わせる",
    import_snap_dmc_note: "各マスに、自由に計算された色の代わりに、内蔵のDMC刺繍糸カラーテーブルの中で最も近い色が割り当てられます。「自分のカラーパレットに合わせる」とは同時に使用できません。",
    edit_dmc_colors_link_title: "DMCカラーテーブルを編集（新しいタブで開きます）",
    import_note_before: "画像は現在のグリッド(",
    import_note_after: "、幅×高さ)に合わせて引き伸ばされ、ピクセルの色に変換されます。",
    export_json_btn: "模様を保存",
    import_json_btn: "模様を読み込む",
    export_png_btn: "PNGとして書き出す",

    group_ansicht_h2: "表示",
    grid10_label: "10本ごとの線を強調表示",
    numbers_label: "行/列番号を表示",
    gridlines_label: "グリッド線を表示",
    ansicht_note: "設定はPNG書き出し時にも適用されます。",

    board_aria: "編み物グリッド",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · ブラウザ上でローカルに動作し、サーバー接続は不要です",

    confirm_clear_all: "本当にパターン全体を消去しますか?",
    err_file_read: "ファイルを読み込めませんでした: ",
    err_image_load: "画像を読み込めませんでした。",
    err_invalid_format: "無効な形式です",
  },

  ko: {
    meta_title: "Julia’s 뜨개질 패턴 편집기",
    app_title: "Julia’s 뜨개질 패턴 편집기",
    app_title_short: "🧶 뜨개질 패턴 편집기",
    eyebrow: "코 그리드",
    header_hint: "그리드를 만들고 코에 색을 입혀 패턴을 이미지나 파일로 저장하세요",
    header_toggle_collapse: "헤더 접기",
    header_toggle_expand: "헤더 펼치기",

    lang_select_aria: "언어 선택",

    sidebar_aria: "설정",
    sidebar_toggle_collapse: "사이드바 접기",
    sidebar_toggle_expand: "사이드바 펼치기",
    panel_icons_aria: "접힌 사이드바",

    icon_werkzeuge_title: "도구",
    icon_werkzeuge_aria: "도구 표시",
    icon_raster_title: "그리드",
    icon_raster_aria: "그리드 설정 표시",
    icon_farbe_title: "색상",
    icon_farbe_aria: "색상 설정 표시",
    icon_muster_title: "패턴",
    icon_muster_aria: "패턴 작업 표시",
    icon_ansicht_title: "보기",
    icon_ansicht_aria: "보기 설정 표시",

    group_werkzeuge_h2: "도구",
    group_raster_h2: "그리드",
    rgb_values_label: "색상 값을 RGB로 표시",
    rows_label: "단(높이)",
    cols_label: "코(너비)",
    cellsize_label: "셀 크기",
    build_btn: "그리드 만들기",
    raster_note: "주의: 그리드를 변경하면 이미 설정된 색상이 초기화됩니다.",

    group_farbe_h2: "색상",
    current_color_label: "현재 색상",
    swatches_aria: "최근 사용한 색상",
    eraser_btn: "지우개",

    tool_label: "도구",
    tooltoggle_aria: "도구 선택",
    tool_paint_title: "펜(자유롭게 그리기)",
    tool_paint_label: "펜",
    tool_area_title: "영역을 선택하고 채우기",
    tool_area_label: "영역",
    tool_pipette_title: "스포이드(패턴에서 색상 추출)",
    tool_pipette_label: "스포이드",
    werkzeug_note: "영역 모드: 모서리를 클릭하고 사각형을 드래그한 뒤 놓으면 채워집니다. 스포이드는 셀을 클릭해 색상을 가져옵니다.",

    cell_bg_label: "기본 셀 배경색",
    cell_bg_note: "아직 칠하지 않은 셀의 배경색입니다. 의도적으로 흰색으로 칠한 셀과 빈 셀을 구분할 수 있습니다. 설정은 계속 저장됩니다.",
    replace_label: "패턴의 색상 바꾸기",
    replace_old_label: "이전 색상",
    replace_new_label: "새 색상",
    replace_pick_title: "그리드에서 가져오기",
    replace_eraser_label: "새 색상 = 지우개(빈 칸)",
    replace_btn: "색상 바꾸기",
    replace_note: "그리드 전체에서 '이전 색상'인 모든 코를 '새 색상'으로 바꿉니다(‘지우개’가 활성화된 경우 비웁니다).",
    replace_none_found: "이 색상은 현재 패턴에 존재하지 않습니다.",

    note_add_title: "새 메모 만들기",
    note_drag_title: "드래그하여 이동",
    note_delete_title: "메모 삭제",
    note_style_title: "스타일 편집",
    note_resize_title: "크기 조절",
    note_bg_label: "배경색",
    note_textcolor_label: "글자색",
    note_fontsize_label: "글자 크기",

    bgimage_btn_title: "배경 이미지",
    bgimage_choose: "이미지 선택 …",
    bgimage_hide: "이미지 숨기기",
    bgimage_show: "이미지 표시",
    bgimage_remove: "이미지 제거",

    icon_farbtabelle_title: "색상표",
    icon_farbtabelle_aria: "색상표 표시",
    group_farbtabelle_h2: "색상표",
    palette_intro_note: "'최근 사용한 색상'과 별개로 최대 49개의 색상을 저장하는 고정 팔레트입니다. 별도의 파일로 저장됩니다.",
    palette_add_color_label: "새 색상",
    palette_pick_title: "그리드에서 추출",
    palette_add_btn: "색상표에 추가",
    palette_add_grid_colors_btn: "그리드 색상 모두 추가",
    palette_add_grid_colors_empty_msg: "그리드에 아직 칠해진 칸이 없습니다.",
    palette_add_grid_colors_full_msg: "{added}개 색상 추가됨, {skipped}개 건너뜀 - 색상표가 가득 찼습니다(49개). 공간을 확보한 후 다시 시도해 주세요.",
    palette_add_grid_colors_none_new_msg: "그리드의 모든 색상이 이미 색상표에 있습니다.",
    palette_add_grid_colors_success_msg: "{added}개 색상을 색상표에 추가했습니다.",
    palette_count_label: "색상표",
    palette_grid_aria: "색상표",
    palette_usage_note: "색상을 클릭하면 펜에 적용됩니다. \"×\"를 클릭하면 색상표에서 제거됩니다.",
    palette_export_btn: "색상표 저장",
    palette_import_btn: "색상표 불러오기",
    palette_clear_btn: "색상표 비우기",
    palette_dmc_names_btn: "🧵 DMC 이름 목록",
    dmc_names_modal_title: "DMC 이름 목록",
    dmc_names_modal_close_aria: "닫기",
    dmc_names_modal_intro: "색상표의 각 색상에 가장 가까운 활성 DMC 실을 표시합니다.",
    dmc_names_source_label: "출처",
    dmc_names_count_label: "색상 수",
    dmc_names_sort_label: "정렬 기준",
    dmc_names_sort_original: "원래 순서",
    dmc_names_sort_code: "DMC 번호",
    dmc_names_sort_name: "이름",
    dmc_names_sort_hue: "색조",
    dmc_names_sort_light: "밝기",
    dmc_names_copy_btn: "📋 목록 복사",
    dmc_names_download_btn: "💾 텍스트 파일로 저장",
    dmc_names_download_html_btn: "🎨 색상 포함 HTML로 저장",
    dmc_names_no_match: "활성화된 DMC 실을 찾을 수 없습니다",
    dmc_names_palette_empty: "색상표가 아직 비어 있습니다.",
    dmc_names_copied_confirm: "✓ 복사됨",
    dmc_names_copy_fallback_prompt: "복사에 실패했습니다 - 아래 텍스트를 선택하여 수동으로 복사하세요:",
    palette_full_msg: "색상표가 가득 찼습니다(최대 49개). 먼저 색상을 하나 제거해 주세요.",
    palette_remove_title: "색상표에서 제거",
    palette_empty_hint: "아직 색상표에 색상이 없습니다. 위의 스포이트나 색상 선택기로 색상을 추가하세요.",
    palette_confirm_clear: "정말 전체 색상표를 비우시겠습니까?",
    page_zoom_aria: "페이지 확대/축소",
    header_saveload_aria: "패턴 저장 / 불러오기 (헤더)",
    header_tools_row_aria: "도구 (헤더)",
    page_zoom_out_title: "페이지 축소",
    page_zoom_in_title: "페이지 확대",

    group_muster_h2: "패턴",
    undo_btn: "실행 취소",
    undo_title: "Ctrl+Z",
    redo_btn: "다시 실행",
    redo_title: "Ctrl+Y",
    clear_btn: "전체 지우기",
    import_image_btn: "이미지를 격자에 불러오기",
    import_levels_label: "채널당 색상 단계",
    import_levels_off: "끄기",
    import_levels_note: "미세한 중간 색조를 여러 개 사용하는 대신 뚜렷하게 구분되는 몇 가지 색상 단계로 줄입니다 - 단계가 적을수록 더 거칠어집니다.",
    import_snap_palette_label: "내 색상표에 맞추기",
    import_snap_palette_note: "각 칸에는 자유롭게 계산된 색상 대신 저장된 색상표에서 가장 가까운 색상이 지정됩니다. 색상 단계와 함께 적용됩니다.",
    import_snap_palette_empty_title: "이 옵션을 사용하려면 먼저 색상표에 색상을 추가하세요.",
    import_snap_dmc_label: "DMC 색상표에 맞추기",
    import_snap_dmc_note: "각 칸에는 자유롭게 계산된 색상 대신 내장된 DMC 실 색상표에서 가장 가까운 색상이 지정됩니다. \"내 색상표에 맞추기\"와 동시에 사용할 수 없습니다.",
    edit_dmc_colors_link_title: "DMC 색상표 편집 (새 탭에서 열림)",
    import_note_before: "이미지는 현재 그리드(",
    import_note_after: ", 너비 × 높이)에 맞춰 늘어나고 픽셀 색상으로 변환됩니다.",
    export_json_btn: "무늬 저장",
    import_json_btn: "무늬 불러오기",
    export_png_btn: "PNG로 내보내기",

    group_ansicht_h2: "보기",
    grid10_label: "10번째 줄마다 강조 표시",
    numbers_label: "행/열 번호 표시",
    gridlines_label: "그리드 선 표시",
    ansicht_note: "설정은 PNG 내보내기에도 적용됩니다.",

    board_aria: "뜨개질 그리드",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · 브라우저에서 로컬로 실행되며 서버 연결이 필요하지 않습니다",

    confirm_clear_all: "정말 전체 패턴을 지우시겠습니까?",
    err_file_read: "파일을 읽을 수 없습니다: ",
    err_image_load: "이미지를 불러올 수 없습니다.",
    err_invalid_format: "잘못된 형식입니다",
  },

  zh_CN: {
    meta_title: "Julia’s 编织图案编辑器",
    app_title: "Julia’s 编织图案编辑器",
    app_title_short: "🧶 编织图案编辑器",
    eyebrow: "针目网格",
    header_hint: "创建网格,为针目上色,将图案保存为图片或文件",
    header_toggle_collapse: "收起页眉",
    header_toggle_expand: "展开页眉",

    lang_select_aria: "选择语言",

    sidebar_aria: "设置",
    sidebar_toggle_collapse: "收起侧边栏",
    sidebar_toggle_expand: "展开侧边栏",
    panel_icons_aria: "已收起的侧边栏",

    icon_werkzeuge_title: "工具",
    icon_werkzeuge_aria: "显示工具",
    icon_raster_title: "网格",
    icon_raster_aria: "显示网格设置",
    icon_farbe_title: "颜色",
    icon_farbe_aria: "显示颜色设置",
    icon_muster_title: "图案",
    icon_muster_aria: "显示图案操作",
    icon_ansicht_title: "视图",
    icon_ansicht_aria: "显示视图设置",

    group_werkzeuge_h2: "工具",
    group_raster_h2: "网格",
    rgb_values_label: "以RGB显示颜色值",
    rows_label: "行数(高度)",
    cols_label: "针数(宽度)",
    cellsize_label: "格子大小",
    build_btn: "创建网格",
    raster_note: "注意:更改网格会重置已设置的颜色。",

    group_farbe_h2: "颜色",
    current_color_label: "当前颜色",
    swatches_aria: "最近使用的颜色",
    eraser_btn: "橡皮擦",

    tool_label: "工具",
    tooltoggle_aria: "选择工具",
    tool_paint_title: "画笔(自由绘制)",
    tool_paint_label: "画笔",
    tool_area_title: "选择并填充区域",
    tool_area_label: "区域",
    tool_pipette_title: "吸管(从图案中取色)",
    tool_pipette_label: "吸管",
    werkzeug_note: "区域模式:点击一个角,拖出矩形,松开即可填充。使用吸管点击格子即可获取该颜色。",

    cell_bg_label: "默认单元格背景色",
    cell_bg_note: "尚未上色的单元格的背景颜色。这样可以将刻意涂成白色的图案格与空白格区分开来。此设置会永久保存。",
    replace_label: "替换图案中的颜色",
    replace_old_label: "旧颜色",
    replace_new_label: "新颜色",
    replace_pick_title: "从网格中取色",
    replace_eraser_label: "新颜色 = 橡皮擦(空白)",
    replace_btn: "替换颜色",
    replace_note: "将整个网格中所有“旧颜色”的针目替换为“新颜色”(若启用“橡皮擦”则清空)。",
    replace_none_found: "当前图案中不存在该颜色。",

    note_add_title: "创建新便签",
    note_drag_title: "拖动以移动",
    note_delete_title: "删除便签",
    note_style_title: "编辑样式",
    note_resize_title: "调整大小",
    note_bg_label: "背景色",
    note_textcolor_label: "文字颜色",
    note_fontsize_label: "字体大小",

    bgimage_btn_title: "背景图片",
    bgimage_choose: "选择图片…",
    bgimage_hide: "隐藏图片",
    bgimage_show: "显示图片",
    bgimage_remove: "移除图片",

    icon_farbtabelle_title: "调色板",
    icon_farbtabelle_aria: "显示调色板",
    group_farbtabelle_h2: "调色板",
    palette_intro_note: "一个固定的永久调色板（最多49种颜色），独立于\"最近使用的颜色\"。保存到独立的文件中。",
    palette_add_color_label: "新颜色",
    palette_pick_title: "从网格中拾取",
    palette_add_btn: "添加到调色板",
    palette_add_grid_colors_btn: "添加所有网格颜色",
    palette_add_grid_colors_empty_msg: "网格中还没有已上色的格子。",
    palette_add_grid_colors_full_msg: "已添加 {added} 种颜色，跳过 {skipped} 种 - 调色板已满（49 种颜色）。请先腾出空间再重试。",
    palette_add_grid_colors_none_new_msg: "网格中的所有颜色都已在调色板中。",
    palette_add_grid_colors_success_msg: "已将 {added} 种颜色添加到调色板。",
    palette_count_label: "调色板",
    palette_grid_aria: "调色板",
    palette_usage_note: "点击颜色即可在画笔中启用。点击\"×\"可将其从调色板中移除。",
    palette_export_btn: "保存调色板",
    palette_import_btn: "加载调色板",
    palette_clear_btn: "清空调色板",
    palette_dmc_names_btn: "🧵 DMC 名称列表",
    dmc_names_modal_title: "DMC 名称列表",
    dmc_names_modal_close_aria: "关闭",
    dmc_names_modal_intro: "为调色板中的每种颜色显示最接近的已启用 DMC 绣线。",
    dmc_names_source_label: "来源",
    dmc_names_count_label: "颜色数量",
    dmc_names_sort_label: "排序方式",
    dmc_names_sort_original: "原始顺序",
    dmc_names_sort_code: "DMC 编号",
    dmc_names_sort_name: "名称",
    dmc_names_sort_hue: "色相",
    dmc_names_sort_light: "明度",
    dmc_names_copy_btn: "📋 复制列表",
    dmc_names_download_btn: "💾 另存为文本文件",
    dmc_names_download_html_btn: "🎨 另存为带颜色的 HTML",
    dmc_names_no_match: "未找到已启用的 DMC 绣线",
    dmc_names_palette_empty: "调色板目前为空。",
    dmc_names_copied_confirm: "✓ 已复制",
    dmc_names_copy_fallback_prompt: "复制失败 - 请手动选择并复制以下文本：",
    palette_full_msg: "调色板已满（最多49种颜色）。请先移除一种颜色。",
    palette_remove_title: "从调色板中移除",
    palette_empty_hint: "调色板中还没有颜色。请使用上方的吸管或取色器添加颜色。",
    palette_confirm_clear: "确定要清空整个调色板吗?",
    page_zoom_aria: "页面缩放",
    header_saveload_aria: "保存 / 加载图案（页眉）",
    header_tools_row_aria: "工具（页眉）",
    page_zoom_out_title: "缩小页面",
    page_zoom_in_title: "放大页面",

    group_muster_h2: "图案",
    undo_btn: "撤销",
    undo_title: "Ctrl+Z",
    redo_btn: "重做",
    redo_title: "Ctrl+Y",
    clear_btn: "清空全部",
    import_image_btn: "将图片载入网格",
    import_levels_label: "每通道色阶",
    import_levels_off: "关闭",
    import_levels_note: "将颜色深度减少为几个清晰可辨的色阶，而不是许多细微的中间色调——色阶越少，效果越粗糙。",
    import_snap_palette_label: "对齐到我的调色板",
    import_snap_palette_note: "每个格子将使用已保存调色板中最接近的颜色，而不是自由计算的颜色。此选项会在色阶设置的基础上叠加应用。",
    import_snap_palette_empty_title: "请先在调色板中添加颜色以使用此选项。",
    import_snap_dmc_label: "对齐到 DMC 色卡",
    import_snap_dmc_note: "每个格子将使用内置 DMC 绣线色卡中最接近的颜色，而不是自由计算的颜色。与\"对齐到我的调色板\"互斥，不能同时启用。",
    edit_dmc_colors_link_title: "编辑 DMC 色卡（在新标签页中打开）",
    import_note_before: "图片将被拉伸以适应当前网格(",
    import_note_after: ",宽×高),并转换为像素颜色。",
    export_json_btn: "保存图案",
    import_json_btn: "载入图案",
    export_png_btn: "导出为PNG",

    group_ansicht_h2: "视图",
    grid10_label: "每10条线高亮显示",
    numbers_label: "显示行/列编号",
    gridlines_label: "显示网格线",
    ansicht_note: "设置同样适用于PNG导出。",

    board_aria: "编织网格",

    footer_before: "(C) 2026",
    footer_after: "· strickmuster.html · 在浏览器中本地运行,无需服务器连接",

    confirm_clear_all: "确定要清空整个图案吗?",
    err_file_read: "无法读取文件: ",
    err_image_load: "无法加载图片。",
    err_invalid_format: "格式无效",
  },

};

// Sprachen, die aktuell in der Auswahl auftauchen (siehe select#langSelect).
// Sobald weitere Sprachen oben ergänzt sind, hier + im <select> freischalten.
const I18N_AVAILABLE = ['de', 'en', 'ru', 'es', 'fr', 'ja', 'ko', 'zh_CN'];
